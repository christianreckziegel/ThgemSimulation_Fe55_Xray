...
int main(int argc, char * argv[]) {
  ...
  sensor->SetArea(-20., -20., -10., 20.,  52.,  10.);
  ...
  for (int i = iniAvalanche; i < maxAvalanche; i++) {
    ...
    y0=electron.y;
    //ei = electron.energy;
    //ei=4.60764*1000.0; //monoEnergy in eV
    ei = 0.;
  ...
  //TCanvas* cFEMesh = new TCanvas("FEMesh", "FE Mesh visualization");
  ViewFEMesh* meshView = new ViewFEMesh();
  //meshView->SetCanvas(cFEMesh);
  meshView->SetArea(xmin,ymin,zmin,xmax,ymax,zmax);
  //meshView->SetArea(xi-0.2, yi-0.5, zi-0.5, xi+0.2, yi+0.5, zi+0.2);
  //meshView->SetArea(-0.2, -0.5, -0.2, 0.2,  0.5,  0.2);
  meshView->SetComponent(elm);
  // x-z projection
  meshView->SetPlane(0, -1, 0, xi, yi, zi);
  //meshView->SetPlane(0, -1, 0, xi, yi, zi);
  //meshView->SetPlane(0, -1, 0, 0.025, 0., 0.); //direção do plano de visualização x=0.025
  meshView->SetFillMesh(true); 
  // Set the color of the bodies.
  meshView->SetColor(1,kYellow + 3);
  meshView->SetColor(2,kGray); //cor do kapton
