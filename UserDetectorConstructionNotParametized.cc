...
G4VPhysicalVolume* UserDetectorConstruction::Construct()
{
    ...
    G4Box* gasBox =
        new G4Box("Gas",                   //its name
                  internalEdge/2., internalEdge/2., (driftThick+inductionThick)/2.);     //its size
    ...
    */
    ...
    G4Box* gasBox =
        new G4Box("Gas",                   //its name
                  5*cm, 5*cm, 2.5*cm);     //its size
    //G4Box* ThickGEMBox = new G4Box("ThickGEMBox", 5*cm, 5*cm, 0.25*mm);
    //G4SubtractionSolid* gasBox = new G4SubtractionSolid("BigGasBox-ThickGEMBox", BigGasBox, ThickGEMBox);
    G4LogicalVolume* logicalGasBox =
        new G4LogicalVolume(gasBox,           //its solid
                            gas,                    //its material
                            "Gas");        //its name
    /*new G4PVPlacement(0,                            //no rotation
                      G4ThreeVector(0,0,0*mm),      //at (0,0,51*mm) or (0,0,-1.75*mm)
                      logicalGasBox,          //its logical volume
                      "Gas",               //its name
                      ...
                      true);         */               //check overlaps
    // Definig walls for the gaseous volume with boolean operation
    G4Box* BigBox = new G4Box("Walls", 5.3*cm, 5.3*cm, 2.5*cm);
    G4SubtractionSolid* wallBox = new G4SubtractionSolid("BigBox-GasBox", BigBox, gasBox);
    G4LogicalVolume* logWallBox = new G4LogicalVolume(wallBox,
    						      kapton,
    						      "Walls");
    ...
    new G4PVPlacement(0,                            //no rotation
                      ...
                      logicalGasBox,          //its logical volume
                      "Gas",               //its name
                      logWallBox,                 //its mother volume (World)
                      false,                        //no boolean operation
                      0,                            //copy number
                      true);                        //check overlaps
    
    // Defining upper cover made of kapton
    G4Box* kaptUpCover = new G4Box("KaptonUpperCover", 5.3*cm,5.3*cm,0.05*mm);
    G4LogicalVolume* logKaptUpCover = new G4LogicalVolume(kaptUpCover, kapton, "KaptonUpperCover");
    G4PVPlacement* kaptUpCoverPosition = new G4PVPlacement(0,
    							   G4ThreeVector(0,0,2.505*cm), // 2.5cm + 0.005cm (0.05mm)
    							   logKaptUpCover,
    							   "KaptonUpperCover",
    							   logicalWorld,
    							   false,
    ...
    G4Box* CuLowCover = new G4Box("CopperLowerCover", 5.3*cm,5.3*cm,1.5*mm);
    ...
    G4PVPlacement* CuLowCoverPosition = new G4PVPlacement(0,
    							   G4ThreeVector(0,0,-2.65*cm),// -2.5cm - 0.15cm
    							   logCuLowCover,
    							   "CopperLowerCover",
    							   logicalWorld,
    							   false,
    ...
    // Defining ThickGEM
    //copper upper cover
    G4Box* coppUpTG = new G4Box("CopperUpperTGEM", 5.*cm,5.*cm,0.005*mm);
    ...
    G4PVPlacement* coppUpTGPosition = new G4PVPlacement(0,
    							   ...
    							   true);
    //kapton middle layer
    G4Box* kapTG = new G4Box("KaptonMiddleLayer", 5.*cm,5.*cm,0.24*mm);
    ...
    G4PVPlacement* kapTGPosition = new G4PVPlacement(0,
    							   ...
    							   true);
    //copper lower cover
    G4Box* coppLowTG = new G4Box("CopperLowerTGEM", 5.*cm,5.*cm,0.005*mm);
